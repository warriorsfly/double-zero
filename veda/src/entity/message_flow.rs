...
impl ToRedisArgs for MessageFlow {
        ...
        W: ?Sized + redis::RedisWrite,
    {
        match self {
            MessageFlow::Evn(ev) => {
                "event".write_redis_args(out);
                ev.write_redis_args(out);
            }

            MessageFlow::Text(txt) => {
                "txt".write_redis_args(out);
                txt.write_redis_args(out);
            }

            MessageFlow::Url(url) => {
                "url".write_redis_args(out);
                url.write_redis_args(out);
            }
        }
    }
...
impl FromRedisValue for MessageFlow {
    fn from_redis_value(v: &redis::Value) -> redis::RedisResult<MessageFlow> {
        match *v {
