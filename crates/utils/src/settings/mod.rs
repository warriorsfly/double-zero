...
impl Settings {
    ...
    ///
    /// Note: The env var `double-zero-DATABASE_URL` is parsed in
    /// `double-zero-db_schema/src/lib.rs::get_database_url_from_env()`
    /// Warning: Only call this once.
    pub fn init() -> Result<Self, DoubleZeroError> {
    ...
    /// Returns something like `http://localhost` or `https://double-zero.ml`,
    ...
    /// variable will be like `double-zero-alpha:8541`. This method removes the port and returns
    /// `double-zero-alpha` instead. It has no effect in production.
    ...
    pub fn save_config_file(data: &str) -> Result<String, DoubleZeroError> {
